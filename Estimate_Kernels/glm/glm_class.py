...
class GLM:
    def __init__(
        ...
        lambda_series = np.logspace(-4, -1, 40), #10.0 ** np.linspace(3, -6, 10), 10**np.linspace(-6, -10, 20), 10.0 ** np.linspace(-1, -8, 30),
        l1_ratio=0.5,
        smooth_strength=0.0,
        optimizer="adam",
        learning_rate=1e-3,
        ...
        if activation == "linear":
            ...
            self.act_func = lambda z: torch.exp(torch.clamp(z, min=-20, max=20))
        elif activation == "relu":
            self.act_func = lambda z: torch.relu(z)
        elif activation == "softplus":
            self.act_func = lambda z: torch.softplus(z)
        elif activation == "sigmoid":
            self.act_func = lambda z: torch.sigmoid(z)

        # select regularization
        if self.regularization == "elastic_net":
            self.reg_func = lambda w: (
                (1.0 - self.l1_ratio) * torch.sum(torch.square(w) / 2.0)
                + self.l1_ratio * torch.sum(torch.abs(w))
            )

        elif self.regularization == "group_lasso":
            self.reg_func = lambda w, grouping_mat, feature_group_size: torch.sum(
                torch.sqrt(grouping_mat @ torch.square(w)) 
                * torch.sqrt(feature_group_size)[:, None]
            )

        if self.smooth_strength > 0.0:
            self.smooth_reg_func = lambda w, P: self.smooth_strength * torch.einsum(
                "ij,ik,kj->", w, P, w
            )
        ...
        if np.logical_and(self.loss_type == "poisson", self.activation == "exp"):
            ...
            self.loss_func = torch.nn.PoissonNLLLoss(log_input=True, reduction='sum')

        elif np.logical_and(self.loss_type == "poisson", self.activation != "exp"):
            self.loss_func = lambda Y, Y_hat, Y_act: torch.sum(
                Y_act - Y * torch.log(stable(Y_act))
            )

        elif self.loss_type == "gaussian":
            self.loss_func = lambda Y, Y_hat, Y_act: torch.sum(torch.square(Y - Y_act))

        elif np.logical_and(
            self.loss_type == "binominal", self.activation == "sigmoid"
        ):
            # use pre-activation value with tf.nn.sigmoid_cross_entropy_with_logits
            self.loss_func = lambda Y, Y_hat, Y_act: F.binary_cross_entropy_with_logits(Y_hat, Y, reduction='sum')
    ...
    def forward(self, X, w, w0):
        ...
        device = w.device  # assume weights already on correct device
        X = X.to(device)
        w0 = w0.to(device)

        #Y_hat = torch.clamp(X @ w + w0, min=-20, max=20)
        
        pre = X @ w + w0
        Y_hat = torch.clamp(pre, -20, 20) if self.activation in ('exp','sigmoid','softplus') else pre
    ...
    def fit(
        ...
        if self.regularization == "group_lasso":
            ...
            self.feature_group_size = torch.tensor(
                feature_group_size, dtype=torch.float32
            )
        ...
        if self.smooth_strength > 0.0:
            ...
            self.prior_matrix = torch.tensor(prior_matrix, dtype=torch.float32)
        ...
        if initial_w0 is not None:
            ...
            initial_w0 = torch.normal(mean=1e-5, std=1e-5, size=(1, self.n_responses))
        ...
        if initial_w is not None:
            ...
            initial_w = torch.normal(mean=1e-5, std=1e-5, size=(self.n_features, self.n_responses))

        # initialize variables
        w0 = torch.nn.Parameter(initial_w0)
        w = torch.nn.Parameter(initial_w)
    ...
    def _compute_loss(self, Xt, Yt, w, w0, lambda_index):
        ...
        if isinstance(self.loss_func, torch.nn.PoissonNLLLoss) and self.loss_func.log_input:
            poisson_term = self.loss_func(Y_hat, Yt)
        else:
            poisson_term = self.loss_func(Yt, Y_hat, Y_act)

        loss = poisson_term / n_t / self.n_responses

        # compute loss
        #loss = self.loss_func(Yt, Y_hat, Y_act) / n_t / self.n_responses
        #loss = self.loss_func(Y_hat, Yt)  / n_t / self.n_responses
        #loss = self.loss_func(Y_act, Yt) / n_t / self.n_responses
    ...
    def _fit(self, Xt, Yt, w, w0, avg_dev, prev_w_series=None, verbose=True):
        ...
        if self.optimizer == "adam":
            opt = torch.optim.Adam([w0, w], lr=self.learning_rate)
        elif self.optimizer == "sgdm":
            opt = torch.optim.SGD([w0, w], lr=self.learning_rate, momentum=self.momentum)
        ...
        if prev_w_series is not None:
            ...
            prev_w0 = torch.tensor(prev_w_series[lambda_index][0], dtype=torch.float32, device=device)
            prev_w = torch.tensor(prev_w_series[lambda_index][1], dtype=torch.float32, device=device)

            # random weights
            random_w0 = torch.normal(mean=1e-5, std=1e-5, size=(1, self.n_responses), device=device)
            random_w = torch.normal(mean=1e-5, std=1e-5, size=(self.n_features, self.n_responses), device=device)
            ...
            if loss_current > loss_random and loss_current > loss_prev:
                if loss_prev < loss_random:
                    w0.data.copy_(prev_w0)
                    w.data.copy_(prev_w)
                else:
                    w0.data.copy_(random_w0)
                    w.data.copy_(random_w)
        ...
        while True:
            ...
            loss_value = loss_this_iter.item()
            assert not torch.isnan(loss_this_iter), "Loss is nan -- check."
            ...
            if (iter_this_lambda % self.num_iter_check) == 0 and (
                ...
                loss_diff = np.mean(-np.diff(loss_trace[-self.num_iter_check:]))

                if (abs(loss_diff) < self.convergence_tol * avg_dev) or (
                    iter_this_lambda >= self.max_iter_per_lambda
                ):
                    if verbose:
                        if iter_this_lambda >= self.max_iter_per_lambda:
                            print(
                                f"Fitting with Lambda {lambda_index} iter {iter_this_lambda} did not converge (loss diff = {loss_diff:.8f})"
                            )
                        else:
                            print(
                                f"Fitting with Lambda {lambda_index} iter {iter_this_lambda} converged (loss diff = {loss_diff:.8f})"
                            )

                    w_series.append([w0.detach().clone().cpu().numpy(), w.detach().clone().cpu().numpy()])
                    lambda_index += 1
                    iter_this_lambda = 0
                    ...
                    if lambda_index < self.lambda_series.shape[0]:
                        if prev_w_series is not None:
                            ...
                            random_w0 = 1e-5 + 1e-5 * torch.randn_like(w0)
                            random_w = 1e-5 + 1e-5 * torch.randn_like(w)
                        ...
                        # Reset optimizer (PyTorch doesn't require re-instantiating it unless weights change)
                        # But this is included here for parity
                        if self.optimizer == "adam":
                            opt = torch.optim.Adam([w0, w], lr=self.learning_rate)
                        elif self.optimizer == "sgdm":
                            opt = torch.optim.SGD([w0, w], lr=self.learning_rate, momentum=self.momentum)
                    ...
                    if verbose:
                        print(
                            f"Lambda {lambda_index} iter {iter_this_lambda} loss: {loss_value:.8f} diff: {loss_diff:.8f}"
                        )
    ...
    def _calculate_fit_quality(self, X, Y):
        ...
        for idx, w in enumerate(self.w_series):
            # make predictions
            prediction_tensor, _ = self.forward(X, w[1], w[0])
    ...
    def select_model(self, X_val, Y_val, min_lambda=1e-5, make_fig=True):
        """
    ...
    def _model_selection_plot(
        ...
        plt.tight_layout()
        #plt.savefig("/n/home07/blanchemarion/sparseness/results/model_selection_plot.png")
        plt.show()
    ...
    def predict(self, X):
        ...
        # Convert inputs to tensor and move to appropriate device
        if isinstance(X, np.ndarray):
            X = torch.tensor(X, dtype=torch.float32)
        device = X.device

        w = torch.tensor(self.selected_w, dtype=torch.float32, device=device)
        w0 = torch.tensor(self.selected_w0.reshape(1, -1), dtype=torch.float32, device=device)

        # make prediction with forward model
        Y_act, _ = self.forward(X, w, w0)
        Y_pred = Y_act.detach().cpu().numpy()
        return Y_pred
    ...
    def evaluate(self, X_test, Y_test, make_fig=True):
        ...
        for idx in range(self.n_responses):
            best_frac_deviance, best_d_model, best_d_null = deviance(
                prediction[:, idx][:, np.newaxis], Y_test_np[:, idx][:, np.newaxis], loss_type=self.loss_type
            )
            """best_frac_deviance, best_d_model, best_d_null = deviance(
                [prediction[:, idx]], [Y_test_np[:, idx]], loss_type=self.loss_type
            )"""
        ...
        if make_fig:
            ...
            plt.tight_layout()
            #plt.savefig("/n/home07/blanchemarion/sparseness/results/fig_evaluate.png")
            plt.show()
...
class GLM_CV(GLM):
    def __init__(
        ...
        loss_type="gaussian",
        regularization="elastic_net",
        lambda_series= np.logspace(-4, -1, 40), #10.0 ** np.linspace(3, -6, 10), 10.0 ** np.linspace(-1, -8, 30),
        l1_ratio=0.5,
        smooth_strength=0.0,
        optimizer="adam",
        learning_rate=1e-3,
    ...
    def fit(
        ...
        if self.regularization == "group_lasso":
            ...
            group_matrix = make_group_matrix(feature_group_size)
            self.group_matrix = torch.tensor(group_matrix, dtype=torch.float32, device=device)
            self.feature_group_size = torch.tensor(feature_group_size, dtype=torch.float32, device = device)
        ...
        if self.smooth_strength > 0.0:
            ...
            prior_matrix = make_prior_matrix(feature_group_size)
            self.prior_matrix = torch.tensor(prior_matrix, dtype=torch.float32, device=device)
        ...
        if initial_w0 is not None:
            ...
            initial_w0 = torch.normal(mean=1e-5, std=1e-5, size=(1, self.n_responses)).to(device)
        ...
        if initial_w is not None:
            ...
            initial_w = torch.normal(mean=1e-5, std=1e-5, size=(self.n_features, self.n_responses)).to(device)

        # initialize variables
        w0 = torch.nn.Parameter(initial_w0.to(device))
        w = torch.nn.Parameter(initial_w.to(device))
        ...
        self.Y_fit = Y.cpu().numpy() if isinstance(Y, torch.Tensor) else Y
        ...
        for n_fold in range(self.n_folds + 1):  # when n_fold == self.n_folds, the fit is using all datapoints
            ...
            X_train = Xt[train_idx[n_fold]]
            Y_train = Yt[train_idx[n_fold]]

            init_w0 = torch.normal(mean=1e-5, std=1e-5, size=(1, self.n_responses), device=device)
            init_w  = torch.normal(mean=1e-5, std=1e-5, size=(self.n_features, self.n_responses), device=device)
            w0 = torch.nn.Parameter(init_w0)
            w  = torch.nn.Parameter(init_w)
            ...
            if n_fold < self.n_folds:
                ...
                X_val = X[these_val_idx, :].clone().detach().to(device)
                Y_val = Y[these_val_idx, :].clone().detach().to(device)
                for this_lambda_idx, this_w in enumerate(w_series):
                    w0_tensor = torch.tensor(this_w[0], dtype=torch.float32, device=device)
                    w_tensor = torch.tensor(this_w[1], dtype=torch.float32, device=device)
                    prediction, _ = self.forward(X_val, w_tensor, w0_tensor)
                    prediction_cpu = prediction.detach().cpu().numpy()
                    _, d_model, _ = deviance(
                        prediction_cpu, Y_val.cpu().numpy(), loss_type=self.loss_type
                    )
            ...
            del X_train, Y_train, w_series  # any large intermediate
    ...
    def select_model(self, se_fraction=1.0, min_lambda=1e-5, make_fig=True):
        """
        ...
        # plot on a linear λ axis so you actually see λ=0
        plt.figure(figsize=(6,4))
        for r in range(all_frac_dev_expl_cv.shape[1]):
            plt.plot(self.lambda_series, all_frac_dev_expl_cv[:,r], color='k', alpha=0.1)
        plt.xscale('log')
        plt.axvline(0.0, color='r')   # sometimes log scale hides 0; just annotate it
        plt.xlabel('lambda')
        plt.ylabel('frac deviance explained (CV)')
        plt.title('Now including the un-penalized fit at λ=0')
        #plt.savefig("/n/home07/blanchemarion/sparseness/results/learning_curve.png")
        plt.show()
        ...
        print(f"all_frac_dev_expl_cv: {all_frac_dev_expl_cv}")
        print(f"selected_frac_dev_expl_cv: {selected_frac_dev_expl_cv}")
    ...
    def make_prediction_cv(self, X_ablated):
        ...
        for n_fold in range(self.n_folds):
            ...
            pred[these_val_frames, :] = this_pred.cpu().numpy()
...
def pointwise_deviance(y_true, y_pred, loss_type="poisson"):
    ...
    if isinstance(y_true, torch.Tensor) and isinstance(y_pred, np.ndarray):
        y_pred = torch.from_numpy(y_pred).to(y_true.device).float()
    elif isinstance(y_pred, torch.Tensor) and isinstance(y_true, np.ndarray):
        y_true = torch.from_numpy(y_true).to(y_pred.device).float()
    elif not isinstance(y_true, (torch.Tensor, np.ndarray)) or not isinstance(y_pred, (torch.Tensor, np.ndarray)):
        print(f"y_true: {type(y_true)}")
        print(f"y_pred: {type(y_pred)}")
        raise TypeError("Inputs must be both torch.Tensors or both np.ndarrays")
    
    assert isinstance(y_true, type(y_pred)), "y_true and y_pred must be of the same type!"

    if isinstance(y_true, torch.Tensor):
        if loss_type == "poisson":
            return 2.0 * (
                y_true * (torch.log(stable(y_true,  eps=1e-10)) - torch.log(stable(y_pred,  eps=1e-10)))
                + y_pred - y_true
            )
            ...
            return 2.0 * (
                -y_true * torch.log(stable(y_pred, eps=1e-10))
                - (1.0 - y_true) * torch.log(stable(1.0 - y_pred, eps=1e-10))
                + y_true * torch.log(stable(y_true))
                + (1.0 - y_true) * torch.log(stable(1.0 - y_true, eps=1e-10))
            )

    elif isinstance(y_true, np.ndarray):
        if loss_type == "poisson":
            return 2.0 * (
                y_true * (np.log(stable(y_true, eps=1e-10)) - np.log(stable(y_pred, eps=1e-10)))
                + y_pred - y_true
            )
        elif loss_type == "gaussian":
            return (y_true - y_pred) ** 2
            ...
            return 2.0 * (
                -y_true * np.log(stable(y_pred, eps=1e-10))
                - (1.0 - y_true) * np.log(stable(1.0 - y_pred, eps=1e-10))
                + y_true * np.log(stable(y_true, eps=1e-10))
                + (1.0 - y_true) * np.log(stable(1.0 - y_true, eps=1e-10))
            )
...
def null_deviance(y, loss_type="poisson"):
    ...
    # If mean_y is a scalar and y is 1D, expand it
    if mean_y.ndim == 0:
        mean_y = mean_y.expand_as(y)

    null_dev = torch.sum(pointwise_deviance(y, mean_y, loss_type=loss_type), dim=0)
    return null_dev
...
def deviance(y_pred, y_true, loss_type="poisson"):
    ...
    assert isinstance(y_true, type(y_pred)), "Mismatch in input types to deviance()"

    is_torch = isinstance(y_true, torch.Tensor)

    if is_torch:
        mean_y = torch.mean(y_true, dim=0)
        """if mean_y.ndim == 0:
            mean_y = mean_y.expand_as(y_true)"""
        d_null = torch.sum(pointwise_deviance(y_true, mean_y, loss_type=loss_type), dim=0)
        d_model = torch.sum(pointwise_deviance(y_true, y_pred, loss_type=loss_type), dim=0)
        frac_dev_expl = 1.0 - d_model / stable(d_null)

        frac_dev_expl[mean_y == 0] = 0  # avoid 0/0

        return frac_dev_expl.cpu().numpy(), d_model.cpu().numpy(), d_null.cpu().numpy()

    else:
        mean_y = np.mean(y_true, axis=0)
        """if mean_y.ndim == 1:
            mean_y = np.full_like(y_true, fill_value=mean_y)"""
