...
def main():
    ...
    for animal in data.keys():
        ...
        for i in range(num_trials):
            ...
            trial_info = {
                 'type':trial_type,
                 'event0_onsets':  [whiff1_onsets[i]] if 0 <= phases1[i] < np.pi else [],
                 'event1_onsets':  [whiff1_onsets[i]] if np.pi <= phases1[i] < 2 * np.pi else [],
                 'event2_onsets':  [whiff2_onsets[i]] if 0 <= phases2[i] < np.pi else [],
                 'event3_onsets':  [whiff2_onsets[i]] if np.pi <= phases2[i] < 2 * np.pi else []
            }
    ...
    save_path = 'Data\processed_ca_resp_to_paired_pulses_data.npy'
